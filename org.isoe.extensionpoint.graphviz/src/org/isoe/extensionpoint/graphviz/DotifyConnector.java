 ...
 *
 */
public class DotifyConnector {
	
	
	private static final boolean LOG = false;
	private static final String EXTENSION_POINT ="org.isoe.diagraph.graphviz";
	
	
	public ILanguageDotifyService getLanguageDotifyService() {
		for (IConfigurationElement cfElement : Platform.getExtensionRegistry().getConfigurationElementsFor(EXTENSION_POINT)) {
			if (cfElement.getAttribute("class") != null) {
				try {
					...
					if (o instanceof ILanguageDotifyService){
						ILanguageDotifyService result=(ILanguageDotifyService) o;
						if (!result.isStub()){
						   if (LOG)
	...
	public IModelDotifyService getModelDotifyService() {
		for (IConfigurationElement cfElement : Platform.getExtensionRegistry().getConfigurationElementsFor(EXTENSION_POINT)) {
			if (cfElement.getAttribute("class") != null) {
				try {
					Object o = cfElement.createExecutableExtension("class");
					if (o instanceof IModelDotifyService){
						IModelDotifyService result=(IModelDotifyService) o;
						if (!result.isStub()){
						   if (LOG)
								  System.out.println(result.getClass().getName());
