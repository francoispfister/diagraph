...
public class DiagraphCommandHandler implements IDiagraphCommandHandler {
	...
	private Map<EClass, List<String>>[] arguments;// createAnnotationMap;
	// private Map<EClass, List<String>> addDetailAnnotationMap;
	...
	/*
	 * @Override public DiagramEditPart getEditPart() { return diagramEditPart;
	 * //FP140502 }
	 */
	...
	public void sGenerateAll() {
		...
		// Set b=a.entrySet();
		// List<String> strs=(List<String>) b.iterator().next();
	...
	public void sGenerateAll(String spov, String tpov, int povid,
			String toHighlight) {
	...
	/*
	 * public DiagraphCommandHandler(IAnnotationHelper helper,boolean dummy) {
	...
	public void generateAll(String srcpov, String trgpov, int povid) {
		...
		for (EClass cl : arguments[0].keySet()) {
			// CreateAnnotationCommand_.sExecute(this, cl,arguments[0].get(cl),
			// helper.getEditor());
			DiagraphAnnotationCreateCommand.sExecute(this,
					"creatediagraphannot", cl, arguments[0].get(cl),
					helper.getEditor());
		}

		if (srcpov != null)
			for (EClass cl : arguments[1].keySet()) {
				AddAnnotationDetailCommand.execute(helper.getDiagraphUtils(),
						cl, srcpov, arguments[1].get(cl).get(0));
			}

		trgpov = normalize(trgpov);
		/*
		 * if (!trgpov.startsWith("view="+DParams.VIEW_PREFIX)) trgpov =
		 * "view="+DParams.VIEW_PREFIX+trgpov; //FP1126xx
		 *
		 * if (trgpov.contains("view="+DParams.VIEW_PREFIX+DParams.VIEW_PREFIX))
		 * trgpov="view="+DParams.VIEW_PREFIX+trgpov.substring(("view="+DParams.
		 * VIEW_PREFIX+DParams.VIEW_PREFIX).length());
		 */
		try {
			changeColor(trgpov, povid); // v is the view prefix
		} catch (Exception e) {
			helper.getControler().cerror("error on generateAll " + e.toString());
		}
		...
		if (arguments == null)
			...
			try {
				for (int i = 0; i < arguments.length; i++) {
					if (arguments[i] != null) {
						Set<EClass> k = arguments[i].keySet();
						if (k != null)
							for (EClass cl : k) {
								String sarg = "";
								List<String> args = arguments[i].get(cl);
								if (args != null)
									for (String arg : args)
										sarg += arg + "-";
								if (LOG) clog("[" + i + "]" + "argument " + cl.getName()
										+ " -args=" + sarg);
							}
					}
				}
			} catch (Exception e) {
				helper.getControler().cerror("error while logArguments");
			}

	}

	public DiagraphCommandHandler(IAnnotationHelper helper) { // FP140521bbb
		this.helper = helper; // FP131114
	...
	public void dropAll() {
		...
		try {
			for (EClass cl : arguments[0].keySet())
				DropAnnotationCommand.execute(this, cl, helper.getEditor());
		} catch (Exception e) {
			helper.getControler().cerror("error while drop all");
		}
	...
	private List<INodeEditPart> getNodeEditparts(EcoreDiagramEditor editor,
	...
	public void delete_(EcoreDiagramEditor editor, EAnnotation eAnnotation) {
		if (!initialized)
	...
	public void removeDiagraphAnnotationsFromDiagram(EcoreDiagramEditor editor,
		...
		if (!initialized)
			initializeElement(editor);
		DeleteAnnotationCommand.executeRemoveFromDiagram(
				getNodeEditparts(editor, toremove), editor);
	}

	@Override
	public void removeDiagraphAnnotationsFromDiagramAndFromModel(
			EcoreDiagramEditor editor_, EAnnotation eAnnotation,
			EAnnotationEditPart toremove) {
		if (!initialized)
			initializeElement(editor_);
		if (toremove != null)
			// DeleteAnnotationCommand.execute_old(helper, eAnnotation,
	...
	public void changeColor(String key, int color_) {
		for (View view : (List<View>) helper.getEditor().getDiagram()
				.getChildren()) {
	...
	private void changeColor(Node node, int color) {
		if (LOG){
			  ...
			  if (node.getElement() instanceof ENamedElement){
			   ...
			   clog("DCH 1 changeColor" +nam+"="+color);
			}
		EList<Style> styl = node.getStyles();
		for (Style style : styl) {
			if (style instanceof FillStyle)
