...
*/
	...
	protected EAnnotation pCreateAnnotationB_(EditPart editPart,
		...
		//Diagram diagram = diagramEditPart!=null?diagramEditPart.getDiagramView():null;
		...
		//claz.getEAnnotations().add(annotation);
		 ...
		 * EcoreElementTypes.EAnnotation_1003);
		 * createElementRequest.setNewElement(annotation); IElementType
		 * elementType = createElementRequest.getElementType();
		 *
		 * ConfigureRequest configureRequest = new
		 * ConfigureRequest(getEditingDomain(), claz, elementType);
		 * configureRequest
		 * .setClientContext(createElementRequest.getClientContext());
	...
	private EAnnotation pCreateAnnotationA(IDiagraphCommandHandler gefHandler, EClass claz, List<String> keys) {
		...
		//Diagram diagram = diagramEditPart!=null?diagramEditPart.getDiagramView():null;
		/*
		 * if (diagram != null) { addEAnnotationReferenceToDiagram(diagram,
		 * claz); }
		 */
		EAnnotation annotation = EcoreFactory.eINSTANCE.createEAnnotation();
		//if (diagram != null)
	...
	//	claz.getEAnnotations().add(annotation);
		...
		EAnnotationCreateCommand annotationCreateCommand = new EAnnotationCreateCommand(
				req);
		ICommandProxy a_ = new ICommandProxy(annotationCreateCommand);

		a_.execute();
		/*
		  ConfigureRequest configureRequest = new ConfigureRequest(getEditingDomain(), annotation, annotationElementType);
		  configureRequest.setClientContext(req.getClientContext());
		  configureRequest.addParameters(req.getParameters());
		  ICommand configureCommand = annotationElementType.getEditCommand(configureRequest);



		ICommandProxy b_ = new ICommandProxy(configureCommand);
...
*/
	...
	public static boolean addEAnnotationReferenceToDiagram(Diagram diagram,
		...
		if (diagram != null) {
			...
			// diagram.getEAnnotation(BelongToDiagramSource);
