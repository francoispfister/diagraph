 ...
 */
package org.isoe.diagraph.langage.internal.utils;
...
//import org.isoe.diagraph.commands.DiagraphCommandHandler_;
...
public class AnnotationUtils implements IAnnotationHelper, IDiagraphUtils {
	...
	/*
	public Map<EClass, List<String>> createAnnotation2( //FP140502
			EditPart editPart,Map<EClass, List<String>> maps[], String srcpov, String pov,
			int povid, EClass target, List<String> details) {
		maps[0].put(target, details);
		arguments = maps;
		new EAnnotation_EditManager(this).pGenerateAll(editPart,srcpov, pov, povid);// controler,editor,
		return maps[0];
	}
*/
	...
	public Map<EClass, List<String>> createNodeAnnotation1(
			Map<EClass, List<String>> annotations, String view,// int povid_,
			EClass target) {
		view = normalizeViewWithPrefix(view);
		//int povid_ = 0;//getControler().getViewId__(view_);//FP140605xxx
		List<String> details = new ArrayList<String>();
		details.add("node");
		//if (povid > 0) {
			// if (pov.startsWith(DParams.VIEW_PREFIX));
		if (!view.isEmpty() && !view.equals("default"))
			details.add("view=" + view);// (pov.startsWith(DParams.VIEW_PREFIX))?"":"");//DParams.VIEW_PREFIX//
			// + pov);
		//}
		// if (level == 1)
		...
		maps[0] = annotations;
		return createAnnotation(maps, null, view, target, details);//FP140502
	}
	...
	private List<EClass> getView_(EPackage pack, String pov_) {
	...
	public void removeVisibleDiagraphAnnotationsFromDiagram() {
		...
		for (Object o : parts) {
			if (o instanceof EAnnotationEditPart) {
				...
				if (eAnnotation.getSource().equals(
						GenericConstants.ANNOT_DIAGRAPH_LITTERAL)) {
					if (isView(eAnnotation, gramgen.getView())) {
						try {
		...
		for (EClassifier eClassifier : editedPackage.getEClassifiers()) {
			if (eClassifier instanceof EClass) {
				...
				for (EAnnotation eAnnotation : ((EClass) eClassifier)
					...
					if (eAnnotation.getSource().equals(
							GenericConstants.ANNOT_DIAGRAPH_LITTERAL)
							&& isView(eAnnotation, gramgen.getView())) {
	...
	public Map<EClass, List<String>> createLinkAnnotation(
		...
		List<String> details = new ArrayList<String>();
		details.add("link");
		//int povid = getControler().getViewId__(view_);
		//int povid = 0;//getControler().getViewId__(view_);//FP140605xxx
		//if (povid > 0)
		if (!view.isEmpty() && !view.equals("default") )
			details.add("view=" + view);
		// if (level == 1) {
		String[] args = getLinkArguments_(target, view);// findLinkArguments(target,pov);
		details.add(args[0]);
		details.add(args[1]);
		details.add(args[2]);
		// }
	...
	public void verifyAfterPov(EClass rootclass) {
		...
		for (EClass tocompl : toCompletge) {
			...
			createGenericAnnotation(annotations, getControler().getNewPov(),
					tocompl, getControler().getNewPovId());// getControler().getNewPovId();
		}
	}
	...
	private String getKeyInSuperClass(EClass claz, String k) {// DiagraphKeywords.UNKNOWN
		...
		for (EAnnotation eAnnotation : annots) {
			if (eAnnotation.getSource().equals(
					GenericConstants.ANNOT_DIAGRAPH_LITTERAL)) {
				Set<String> keys = eAnnotation.getDetails().keySet();
				for (String key : keys) {
					if (key.startsWith(k))
						return key;
				}
	...
	private String[] getNodeArguments(EClass node, String view) {
		List<String> result = new ArrayList<String>();
		EPackage pack = node.getEPackage();
		List<EClass> viu = getView_(node.getEPackage(), view);
		List<String> ncrelations = getNoContRelations(viu, view, node);
		for (String r : ncrelations)
			result.add(DiagraphKeywords.REFERENCE_EQU + r);
		List<String> contrelations = getContRelations(viu, view, node);
	...
	private String[] getLinkArguments_(EClass link, String pov_) {
		String[] result = new String[3];
		EPackage pack = link.getEPackage();
		List<EClass> viu = getView_(link.getEPackage(), pov_);
		List<String> containers = getContainers(viu, pov_, link);
		List<String> relations = getNoContRelations(viu, pov_, link);
...
/*
	...
	public List<EClass> propagateNodes(String view, EClass c) {
		...
		try {
			...
			System.err.println("error on propagate");
		}
		return p;
	...
	private Map<EClass, List<String>> propagateNonVisual_() {
		String language=((EPackage)getControler().getEcoreDiagram().getElement()).getName();
		String currentView=getControler().getCurrentView(4, language,this.getClass().getName());
		...
		for (View gview : (List<View>) getControler().getEcoreDiagram()
			...
			if (gview instanceof Node) {
				...
				if (node.getElement() instanceof EClass) {
					...
					if (!isDiagraphAnnotated(notAnnotated, currentView)) { //FP140611
						// Non visual element
		...
		for (View view : (List<View>) getControler().getEcoreDiagram()
			...
			if (view instanceof Node) {
				...
				if (node.getElement() instanceof EClass) {
					...
					// if (cl.getName().equals("EClass7"))
					// tb = true;
					// String viu = getControler().getCurrentView();
					...
					/*
					 * if (isDiagraphAnnotated(annotated,viu)) { for (EClass
					 * superAnnotated : annotated.getEAllSuperTypes()) {
					 *
					 *
					 *
					 * if (isDiagraphAnnotated(superAnnotated,viu)) { for
					 * (EAnnotation an : superAnnotated .getEAnnotations()) {
					 *
					 *
					 *
					 ...
					 *
					 * if (!clzez.contains(annotated) && annotated!=c)
					 * clzez.add(annotated);
					 *
					 *
					 * }
	...
	private Map<EClass, List<String>> propagateByInheritance_() {
		...
		Diagram  d= getControler().getEcoreDiagram();
		String lang=((EPackage) d.getElement()).getName();
		String viu = getControler().getCurrentView(5,lang,this.getClass().getName()); //FP140611
		//FP140611
		for (View view : (List<View>) d.getChildren()) {
			if (view instanceof Node) {
				...
				if (node.getElement() instanceof EClass) {
					...
					//String viu = getControler().getCurrentView(5,this.getClass().getName());
