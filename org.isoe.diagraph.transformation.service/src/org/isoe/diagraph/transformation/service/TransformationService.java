 ...
 */
package org.isoe.diagraph.transformation.service;
...
import org.isoe.diagraph.transform.version.AbstractDiagraphTransformation;
...
public class TransformationService extends Action implements ITransformationService, DParams { //, IAlignAction


	//contributes to: org.isoe.extensionpoint.similarity.diagraph_similarity

	//com.googlecode.alignment.ui.AlignAction
	...
	private boolean getUris() {
		...
		//metamodelRetriever = controler.getMetamodelRetriever();
		...
		URI uriresult = createplatformResourceModelURI(getRepositoryLocation(), "titi","tata", "toto");

		uris=new URI[5];

		uris[0]=uriresult; //platform:/resource/isoe.bibtex....
	...
	public void run() {
		...
		if (getUris()){
		   AbstractEmfTransformation transfo= new AbstractDiagraphTransformation();
		   transfo.executeOneInOneOut(uris[1],uris[0]);
		} else
	...
	@Override
	public void setLanguageConfiguration(boolean languageSide) {
		this.languageSide = languageSide;
	}
