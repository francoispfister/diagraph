...
public class M0M2cToDot extends DotWriter implements DotConstants, DotVisitable,
GraphVizConverter{

	private CompiledGraph diagraphM2;
	private CompiledGraph diagraphM1;
	private CompiledGraph domainM2;
	...
	public M0M2cToDot(String out, CompiledGraph diagraphM2_,
			CompiledGraph diagraphM1_, CompiledGraph domainM2_) {
		this.outputFile = out;
		this.diagraphM2=diagraphM2_;
		this.diagraphM1=diagraphM1_;
		this.domainM2 = domainM2_;
		concreteSyntax = (DGraph) diagraphM1.getRootObject();
		System.out.println(diagraphM2.getRootObject());//EPackageImpl@7a9f36 (name: diagraph)
		System.out.println(diagraphM1.getRootObject());//l.DGraphImpl@11e86da (graphHandler: n
	}
	...
	public String toDot() {
		...
		dot += diagraphM2.getNodesAsString(0);
		dot += endSubGraph() + "\n";

		dot += startSubGraph(subgraphes++, true);
		dot += m2Label("B" + subgraphes) + ";" + "\n";
		dot += diagraphM2.getNodesAsString(1);
		dot += endSubGraph() + "\n";


		dot += startSubGraph(subgraphes++, true);
		dot += m2Label("C" + subgraphes) + ";" + "\n";
		dot += diagraphM1.getNodesAsString(1);
		dot += endSubGraph() + "\n";


		dot += startSubGraph(subgraphes++, true);
		dot += m2Label("D" + subgraphes) + ";" + "\n";
		dot += domainM2.getNodesAsString(1);
		dot += endSubGraph() + "\n";


		dot += startSubGraph(subgraphes++, true);
		dot += m2Label("E" + subgraphes) + ";" + "\n";
		dot += "blabla\n";
		dot += endSubGraph() + "\n";
		...
		String eas1= diagraphM2.getEdgesAsString(0);
		dot += "/*1b ecoreDiagraphM2Graph Edges*/\n\n";

		edges0 = diagraphM2.getEdges(0);



		String eas2= diagraphM2.getEdgesAsString(1);
		...
		List<EObjectEdge> oes=diagraphM1.getEdges(0);
		...
		dot += diagraphM1.getEdgesAsString(0);

		edges1 = diagraphM1.getEdges(0);



		dot += "/*3 ddomainM2 Edges  */\n\n";
		dot += domainM2.getEdgesAsString(0);

		edges1.addAll(domainM2.getEdges());


		dot += "/*4 diagraphInstance Edges  */\n\n";
