...
public class LanguageDotifyAction extends Action implements DParams, ILanguageDotifyService{

	private static final String LOG_TARGET="m2PackageLabel";

	private IDiagraphControler controler;

	//private IMetamodelRetriever metamodelRetriever_;
	...
	public LanguageDotifyAction() {
		super();
		setText("Dotify");
		setToolTipText("Dotify");
	}
	...
	@Override
	public void runGraphviz(IFile cfile,EPackage pak,URI mUri) {
		  String actionId = DOTIFY_ACTION_PREFIX + controler.getCommandId();//dotifyTypeCombo.getText();
		  try {
			  setRegisteredMetamodelAndinvokeDot(cfile,pak,mUri,actionId);
		 } catch (Exception e) {
	...
	private void setRegisteredMetamodelAndinvokeDot(IFile cfile,EPackage pak, URI mUri,
			String actionId) {
		...
		helper.setCurrentFile(cfile);
		helper.setPackage(pak);
		helper.setModel(mUri);
	...
	private boolean getUris() {
		...
		//metamodelRetriever_  = controler.getMetamodelRetriever();
	...
	public void run() {
		...
		if (getUris()){
		 	runGraphviz(leftFile,leftMM,leftMUri);
		} else
