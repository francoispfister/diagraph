...
<xsl:stylesheet version="2.0"
	...
	xmlns:lambda="http:///classmate.sample"
	extension-element-prefixes="xsltsl-str str mylib">
	<xsl:template match="lambda:ClassmateSystem/school/classrooms[*]/students[1]/@name">
		...
		<xsl:attribute name="name">
         	<xsl:call-template name="mylib:rewriteName">
				...
				<xsl:with-param name="market" select="'azdsfsderty'"/>
			</xsl:call-template>
        </xsl:attribute>
	</xsl:template>
</xsl:stylesheet>
