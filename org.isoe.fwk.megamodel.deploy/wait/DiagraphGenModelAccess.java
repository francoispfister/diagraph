...
/**
 * @author artem
 * @author fpfister  adaptation
 * 
 */
public class DiagraphGenModelAccess implements GenModelAccess {
   
   protected final EPackage model;
   protected final EPackage additionalModel;
   ...
   public DiagraphGenModelAccess(EPackage aModel,EPackage additionalModel) {
      this.model = aModel;
   ...
   public IStatus createDummy(Collection<EPackage> additionalPackages) {
      genModel = new DiagraphDummyGenModel(model, additionalPackages).create();
      confineInResource();
      return Status.OK_STATUS;
   }
