...
public class ModelWorkbenchLauncher extends Action implements
		IJavaLaunchConfigurationConstants, ILauncherSettings, IModelWorkbenchLauncher {

	private String launcherName;
	private IDiagraphControler controler;
	private static final boolean LOG = DParams.ModelWorkbenchLauncher_LOG;

	private boolean silent;
	private int port = -1; //FP140526
...
/*
		...
		*/
		...
		String vmargs_old = "-Dosgi.requiredJavaVersion=1.5 -Duser.name=\""
				+ userName + "\" -Dteam.namespace=\"" + teamNamespace
				+ "\" -Dteam.name=\"" + teamName + "\" -Xms128m -Xmx512m -XX:MaxPermSize=350m -XX:ReservedCodeCacheSize=180m";


		String vmargs = "-Dosgi.requiredJavaVersion=1.6 -Duser.name=\""
				+ userName + "\" -Dteam.namespace=\"" + teamNamespace
				+ "\" -Dteam.name=\"" + teamName + "\" "+DParams.LAUNCHER_VM_PARAMS;

		if (port !=-1)
			vmargs += " -Dport="+port; //FP140526
		if (LOG)
			clog("launch with args "+vmargs);
		...
		String mwblaunchmode = DParams.MWB_LAUNCH_MODE;
		DebugUITools.launch(configuration, mwblaunchmode);
